...
let replaceCSSComments = (n) => n.replace(/\s?\/\*.*?\*\//gi, '');
...
let replaceHTMLComments = (n) => {
  let t = n.replace(/^\s?<!--.*?-->\s?|\s?<!--.*?-->/gi, '');
  return t;
}
...
let validateFileType = (n) => (/\.jpg$|\.jpeg$|\.png$/).test(n);
...
let checkYear = (n) => (/^19[0-9]{2,2}$|^20[0-9]{2,2}$|^2100$/).test(n);
...
let checkEmail = (n) => (/^[a-zA-Z0-9_.-]+@[a-zA-Z.0-9]+\.[a-zA-Z]{2,5}$/).test(n);
...
let checkDomainUrl = (n) => (/^(http:\/\/|https:\/\/)[a-zA-Z.0-9_-]+\.[a-zA-Z]{2,5}$/).test(n);
...
function createLinksFromDomains(n) {
  let v = n.match(/(http:\/\/|https:\/\/)[a-zA-Z.0-9_-]+\.[a-zA-Z]{2,5}/g);
  // console.log(v.length);
  // for (let j = 0; j < v.length; j++) {console.log(v[j])}
  if (v === null) { return n };
  for (let i = 0; i < v.length; i++) {
    n = n.replace(/(http:\/\/|https:\/\/)([a-zA-Z.0-9_-]+\.[a-zA-Z]{2,5})/, '<a href="$1!$2" target="_blank">$2</a>');
  }
  n = n.replace(/(http:\/\/|https:\/\/)!([a-zA-Z.0-9_-]+\.[a-zA-Z]{2,5})/g, '$1$2');
  return n;
}
...
// createLinksFromDomains('site.ua text1 https://site.com text2 https://site.com.ua text3 subdomain.my-site.com.ua text4');

// console.log(createLinksFromDomains('Функция получает произвольную строку текста с доменами (один и более), возвращает результат преобразования.'));
